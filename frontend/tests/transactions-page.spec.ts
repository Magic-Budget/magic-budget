...
test("navigate transactions", async ({ page }) => {
  ...
  await expect(
    page.getByRole("cell", { name: "Date" }).isVisible(),
  ).toBeTruthy();
  await expect(
    page.getByRole("cell", { name: "Name" }).isVisible(),
  ).toBeTruthy();
  await expect(
    page.getByRole("cell", { name: "Category" }).isVisible(),
  ).toBeTruthy();
  await expect(
    page.getByRole("cell", { name: "Description" }).isVisible(),
  ).toBeTruthy();
